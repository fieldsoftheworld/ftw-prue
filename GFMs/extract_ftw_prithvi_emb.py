...
def process_country(country: str, base_dir: Path, wrapper):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not in_dir.exists():
            continue
        out_dir = Path(str(in_dir).replace("FTW-Dataset/ftw", "FTW-Prithvi-Embeddings"))
        out_dir = Path(str(out_dir).replace("s2_images", "prithvi_eo_v2"))
...
if __name__ == "__main__":
    ...
    wrapper = PrithviWrapper(weights_path=ckpt_base_dir / "prithvi", do_pool=False).cuda()
