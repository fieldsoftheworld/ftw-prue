...
def process_country(country: str, base_dir: Path, wrapper):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not in_dir.exists():
            continue
        out_dir = Path(str(in_dir).replace("FTW-Dataset/ftw", "FTW-SoftCon-Embeddings"))
        out_dir = Path(str(out_dir).replace("s2_images", "softcon_B13_vits14"))
...
if __name__ == "__main__":
    ...
    wrapper = SoftConWrapper(
        weights_path=ckpt_base_dir / "softcon", size="small", modality="optical", do_pool=False
    ).cuda()
