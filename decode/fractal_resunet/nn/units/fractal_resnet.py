...
class ResNet_v2_block(nn.Module):
    ...
    def __init__(self, nfilters, kernel_size=(3, 3), dilation_rate=(1, 1), 
                 norm_type='BatchNorm', norm_groups=None, ngroups=1, **kwargs):
        super().__init__()
...
class FracTALResNet_unit(nn.Module):
    ...
    def __init__(self, nfilters, ngroups=1, nheads=1, kernel_size=(3, 3), 
                 dilation_rate=(1, 1), norm_type='BatchNorm', norm_groups=None, 
                 ftdepth=5, **kwargs):
        super().__init__()

        self.block1 = ResNet_v2_block(
            nfilters, kernel_size, dilation_rate, 
            norm_type=norm_type, norm_groups=norm_groups, ngroups=ngroups
        )
        self.attn = FTAttention2D(
            nkeys=nfilters, nheads=nheads, kernel_size=kernel_size, 
            norm=norm_type, norm_groups=norm_groups, ftdepth=ftdepth
        )
