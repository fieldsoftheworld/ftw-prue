...
from .transform import ExtentTransform, ResizeTransform, RotationTransform, NormalizeTransform
...
class Normalize(Augmentation):
    """
    Added augmentation: Normalize the input image.
    """

    def __init__(self, mean, std):
        """
        Args:
            mean (tuple): mean values of input image
            std (tuple): std values of input image
        """
        super().__init__()
        self._init(locals())

    def get_transform(self, image):
        return NormalizeTransform(mean=self.mean, std=self.std)