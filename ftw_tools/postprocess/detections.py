...
from ftw_tools.postprocess.intermediate_formats import SemanticOutput, InstanceOutput, PanopticOutput
...
class Detections:
    ...
    @classmethod
    def from_sam(cls, sam_results: list[dict]) -> Detections:
        """
        Creates a Detections instance from Segment Anything Model (SAM) outputs.
        """
        if not sam_results:
            return cls(xyxy=np.empty((0, 4)))

        masks = np.array([res['segmentation'] for res in sam_results])
        xyxy = np.array([res['bbox'] for res in sam_results])
        
        return cls(
            xyxy=xyxy,
            mask=masks,
            confidence=np.array([res['predicted_iou'] for res in sam_results])
        )
