...
class Detections:
    """Minimal Detections class to satisfy Evaluator for semantic masks."""

    def __init__(self, mask: np.ndarray, confidence_map=None):
        self.mask = mask
        ...
        for i in range(1, num_features + 1):
            ...
            if self.confidence_map is not None:
                score = float(np.mean(self.confidence_map[instance_mask]))
            self.confidence.append(score)

    def __len__(self):
        return self.num_features

    def to_binary_mask(self, shape, field_class_id=None):
        return self.mask

    def to_coco_format(self, image_id, start_annotation_id):
        annotations = []
        ann_id = start_annotation_id

        labeled_mask, _ = label(self.mask)
...
class Evaluator:
            ...
            if "coco" in self.metrics:
                ...
                if i < 3:
                    ...
                    if pred_dets.confidence is not None and len(pred_dets.confidence) > 0:
                        ...
                        self.logger.info(
                            f"  - Confidence scores: min={conf_min:.4f}, max={conf_max:.4f}, "
                        ...
                        if unique_vals == 1:
                            ...
                            self.logger.warning(
                                f"    COCO mAP will compute but precision-recall curves won't reflect confidence ordering."
                            )
                        elif unique_vals < len(conf_array) * 0.1:  # Less than 10% unique values
                            self.logger.warning(
                                f"  - WARNING: Only {unique_vals}/{len(conf_array)} unique confidence values."
                            )
    ...
    def _create_coco_gt(self) -> COCO:
        ...
        # add info / licenses section to COCO dataset
        coco_dataset = {
            "info": {
                "description": "FTW segmentation GT",
                "version": "1.0",
                "year": 2025,
            },
            "licenses": [],
