...
class PixelWeightedCE(nn.Module):
    ...
    def __init__(
        ...
        self.blur = K.filters.GaussianBlur2d(kernel_size=self.kernel_size, sigma=self.sigma, border_type="reflect")
...
class logCoshDiceCE(nn.Module):
    def __init__(
        self, weight_ce=0.5, weight_dice=0.5, ignore_index=None, class_weights=None, mode="multiclass", classes=None
    ):
        super().__init__()
        ignore_value = -1000 if ignore_index is None else ignore_index
        self.ce_loss = nn.CrossEntropyLoss(ignore_index=ignore_value, weight=class_weights)
        self.dice_loss = logCoshDice(mode=mode, classes=classes, class_weights=class_weights, ignore_index=ignore_index)
