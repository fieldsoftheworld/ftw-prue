...
try:
    ...
    import weighted_gather_cuda
    import msdetrpc_cuda
except ImportError:
...
class WEIGHTEDGATHERFunction(Function):
    """
    weighted gather function
    """
    @staticmethod
    def forward(ctx, nbhd_idx, weights, feat):
        nbhd_idx = nbhd_idx.contiguous()
        weights = weights.contiguous()
        feat = feat.contiguous()
        if feat.dtype != weights.dtype:
            weights = weights.to(feat.dtype)
        feat_new = weighted_gather_cuda.forward(
            nbhd_idx,
            weights,
            feat)
    ...
    @staticmethod
    def backward(ctx, grad_feat_new):
        outputs = weighted_gather_cuda.backward(
            grad_feat_new.contiguous(), *ctx.saved_tensors)
        d_weights, d_feat = outputs
...
class MSDETRPCFunction(Function):
    """
    deformable multi scale detr point cloud function
    """
    @staticmethod
    def forward(ctx, nn_idx, nn_weight, attn, val):
        nn_idx = nn_idx.contiguous()
        nn_weight = nn_weight.contiguous()
        attn = attn.contiguous()
        val = val.contiguous()
        feat = msdetrpc_cuda.forward(
            nn_idx,
            nn_weight,
            attn,
            val)
    ...
    @staticmethod
    def backward(ctx, grad_feat):
        outputs = msdetrpc_cuda.backward(
            grad_feat.contiguous(), *ctx.saved_tensors)
        d_weight, d_attn, d_val = outputs
        return None, d_weight, d_attn, d_val
