...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.conv2d_s2_ftw = nn.Conv2d(
            4, attn_dim, kernel_size=patch_size, stride=patch_size
        )  # patch_embed layer for FTW dataset
    ...
    def forward(self, x12: Tensor, isl2a: bool = True) -> Tensor:

        B, C, W, H = x12.shape
        device, dtype = x12.device, x12.dtype
        B = len(x12)
        if C == 4:
            x = self.conv2d_s2_ftw(x12).flatten(2).transpose(1, 2)
        elif C == 2:
...
class TerraFM(nn.Module):
    ...
    def prepare_tokens(self, x, isl2a=True):
        B, nc, w, h = x.shape
        x = self.patch_embed(x, isl2a=isl2a)  # patch linear embedding
    ...
    def forward(self, x, return_cls=True, isl2a=True):
        # import code;code.interact(local=dict(globals(), **locals()));
        x = self.prepare_tokens(x, isl2a=isl2a)
    ...
    def get_last_selfattention(self, x, isl2a=True):
        x = self.prepare_tokens(x, isl2a=isl2a)
    ...
    def get_intermediate_layers(self, x, n=1, return_class_token=False, norm=False, isl2a=True):
        x = self.prepare_tokens(x, isl2a=isl2a)
    ...
    def extract_feature(self, images, isl2a=True, return_h_w=True, out_indices=[3, 5, 7, 11]):
        x = self.prepare_tokens(images, isl2a=isl2a)
        output = []
