...
scaled_index_add = None
fmha = None
index_select_cat = None


logger = logging.getLogger("dinov2")


class SoftConWrapper(nn.Module):
    def __init__(
        self,
        weights_path: Path,
        size,
        ...
        if (size == "small") and (modality == "optical"):
            ...
            if load_state:
                checkpoint = torch.load(weights_path / "B13_vits14_softcon.pth", map_location="cpu")
            self.dim = 384
        elif (size == "base") and (modality == "optical"):
            ...
            if load_state:
                checkpoint = torch.load(weights_path / "B13_vitb14_softcon.pth", map_location="cpu")
            self.dim = 768
        elif (size == "small") and (modality == "SAR"):
            self.encoder = vit_small(
                img_size=224,
                patch_size=14,
                in_chans=2,
            ...
            if load_state:
                checkpoint = torch.load(weights_path / "B2_vits14_softcon.pth", map_location="cpu")
            self.dim = 384
        elif (size == "base") and (modality == "SAR"):
            self.encoder = vit_base(
                img_size=224,
                patch_size=14,
                in_chans=2,
            ...
            if load_state:
                checkpoint = torch.load(weights_path / "B2_vitb14_softcon.pth", map_location="cpu")
            self.dim = 768
        else:
            raise ValueError(f"size should be small or base, not {size}")
    ...
    def _num_params(self):
        param_attrs = [self.encoder,]
        total_params = 0
        for model in param_attrs:
            for name, parameter in model.named_parameters():
                if parameter.requires_grad or True:  # Only count trainable parameters
    ...
    def preproccess(self, images):
        ...
        return self.resize(images)  # (bsz, 13, 224, 224)

    def forward(self, s2=None, s1=None, months=None):
        if s2 is None:
            raise ValueError("S2 can't be None for SoftCon")

        # TODO include S1 support too.
        if len(s2.shape) == 5:
            outputs_l: List[torch.Tensor] = []
            for timestep in range(s2.shape[3]):
                image = self.preproccess(s2[:, :, :, timestep])
                output = self.encoder.forward_features(image)["x_norm_patchtokens"]
            ...
            output = self.encoder.forward_features(s2)["x_norm_patchtokens"]
            if self.do_pool:
                output = output.mean(dim=1)
            return output
