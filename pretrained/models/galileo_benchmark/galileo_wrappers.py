...
class BaseGalileoEncoder(nn.Module):
    ...
    def forward(self, datacube):
        ...
        if C == 4:
            s2_13band = self.preprocess_4band_to_13band(x, self.mean, self.std, x.device)
            ...
            s2_13band_a = self.preprocess_4band_to_13band(x_a, self.mean, self.std, x.device)
            s2_13band_b = self.preprocess_4band_to_13band(x_b, self.mean, self.std, x.device)
