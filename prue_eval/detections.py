...
class Detections:
    ...
    image_filename: Optional[str] = None  # Image filename for robust matching
    ...
    def __iter__(
        self,
    ) -> Iterator[Tuple[np.ndarray, Optional[np.ndarray], Optional[float], Optional[int], Optional[Polygon]]]:
        """
        Iterates over the Detections object and yields a tuple of
        (xyxy, mask, confidence, class_id, polygon) for each detection.
        """
    ...
    @classmethod
    def from_semantic_logits(
        cls, semantic_logits: SemanticOutput, field_class_id: int = 1, min_area: int = 0
    ) -> Detections:
        """
        ...
        for geom, val in rasterio.features.shapes(field_mask):
            if val == 1:  # Only process field pixels
                ...
                mask = rasterio.features.rasterize(
                    [shapely_geom], out_shape=field_mask.shape, fill=0, default_value=1, dtype=np.uint8
                )
                masks.append(mask)
    ...
    def from_instance_masks(
        cls, instance_masks: InstanceOutput, min_area: int = 0, score_threshold: float = 0.0
    ) -> Detections:
        """
                                ...
                                try:
                                    ...
                                    if not shapely_geom.is_valid:
                                        ...
                                        if hasattr(shapely_geom, "geoms"):
                                            shapely_geom = next(
                                                (g for g in shapely_geom.geoms if isinstance(g, Polygon)), None
                                            )
