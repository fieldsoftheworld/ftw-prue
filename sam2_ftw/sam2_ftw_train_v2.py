...
CHECKPOINT_PATH = os.environ.get("SAM2_CHECKPOINT_PATH", "checkpoints/sam2.1_hiera_small.pt")
...
def main():
    ...
    dataloader = DataLoader(
        dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, collate_fn=collate_fn, drop_last=True
    )
    ...
    optimizer = torch.optim.AdamW(model.sam_mask_decoder.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast("cuda", enabled=(device == "cuda")):
                ...
                mask_prompt = F.interpolate(gt_masks_flat.unsqueeze(1).float(), size=(256, 256), mode="nearest")
                ...
                focal_loss = -gt_masks_exp * alpha * (1 - pred_prob) ** gamma * torch.log(pred_prob + 1e-5) - (
                    1 - gt_masks_exp
                ) * (1 - alpha) * pred_prob**gamma * torch.log((1 - pred_prob) + 1e-5)
