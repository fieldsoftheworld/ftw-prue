...
def process_country(country: str, base_dir: Path, wrapper):
    ...
    for window in ["window_a", "window_b"]:
        ...
        if not in_dir.exists():
            continue
        out_dir = Path(str(in_dir).replace("FTW-Dataset/ftw", "FTW-CROMA-Embeddings"))
        out_dir = Path(str(out_dir).replace("s2_images", "croma_base"))
...
if __name__ == "__main__":
    ...
    wrapper = CROMAWrapper(weights_path=ckpt_base_dir / "croma", size="base", modality="optical", do_pool=False).cuda()
