    ...
    for country in countries_sorted:
        ...
        for aoi_id in aoi_ids:
            ...
            if os.path.exists(semantic_2_file):
                ...
                print(f"Warning: Semantic mask not found for {country}/{aoi_id}")
                continue
        ...
        # Process valid AOI IDs in sorted order
        for aoi_id in valid_aoi_ids:
...
def evaluate_by_country(
    ...
    Uses semantic masks for object metrics (extracts connected components).
    ...
    for country in unique_countries:
        ...
        # Prepare GT detections: convert instance masks for COCO metrics, empty otherwise
        ...
        if "coco" in metrics and gt_instance_masks is not None:
            ...
            # Empty detections (not used for pixel/object metrics with semantic masks)
        ...
        # Create evaluator: uses semantic masks for object metrics (extracts connected components)
        evaluator = Evaluator(
        ...
        for model_name, pred_detections in country_model_detections.items():
            ...
            # Pixel/object metrics use gt_masks directly; COCO metrics need y_true Detections
