...
def predict_viz(
    cfg: CfgNode, image_path: str, output_dir: str, metadata=None, ground_truth_dir: Optional[str] = None
) -> None:
    """
    ...
    for img_path in image_paths:
        try:
                    ...
                    if gt_path.exists() and json_path.exists():
                        ...
                        if pan_gt is not None:
                            ...
                            if segments_info is not None:
                                ground_truth = {"panoptic_seg": (torch.as_tensor(pan_seg_gt), segments_info)}
            ...
            vis_path = visualizer.visualize_predictions(
                image, predictions, ground_truth=ground_truth, output_dir=output_dir, image_id=img_path.stem
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--coco-root", help="Path to COCO format dataset root containing panoptic_test/ and annotations/"
    )
