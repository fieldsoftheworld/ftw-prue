...
def process_single_geotiff(
    ...
    for i, (chip, (x, y)) in enumerate(tqdm(zip(chips, coords), total=len(chips), desc=f"Processing {geotiff_name}")):
        try:
            ...
            if chip_gdf.empty:
                logger.debug(
                    f"Chip {i} at ({x}, {y}): No polygons after conversion (had {len(filtered_segments)} segments)"
                )
                continue
            ...
            if polygons_before_edge > 0 and polygons_after_edge == 0:
                logger.debug(
                    f"Chip {i} at ({x}, {y}): Lost all {polygons_before_edge} polygons to edge filtering (buffer={edge_buffer}m)"
                )
            elif polygons_after_edge < polygons_before_edge:
                logger.debug(
                    f"Chip {i} at ({x}, {y}): Edge filtering removed {polygons_before_edge - polygons_after_edge}/{polygons_before_edge} polygons"
                )
    ...
    if not chip_geodataframes:
        ...
        logger.warning(
            f"Consider: reducing --edge-buffer (current: {edge_buffer}m), reducing --min-area (current: {min_area}m²), or checking model predictions"
        )
    ...
    try:
        ...
        if not gdf.empty:
            for idx, row in gdf.iterrows():
                ...
                feature = {"type": "Feature", "geometry": mapping(row.geometry), "properties": properties}
...
def main(args):
    ...
    logger.info(
        f"Final metadata - thing_dataset_id_to_contiguous_id: {getattr(metadata, 'thing_dataset_id_to_contiguous_id', 'NOT SET')}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iou-threshold", type=float, default=0.7, help="IoU threshold for merging overlapping predictions"
    )
    parser.add_argument(
        "--edge-buffer",
        type=float,
        default=5.0,
        help="Edge buffer in meters for filtering edge polygons (default: 5.0m, reduce for small chips)",
    )
    parser.add_argument(
        "--min-area-pixels",
        type=int,
