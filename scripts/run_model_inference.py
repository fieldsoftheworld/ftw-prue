...
This script provides a common interface for running inference across different model types
and converting their outputs to the standardized Detections format for evaluation.
"""
...
from converters import (
    ...
    convert_mask2former_panoptic_output,
)
...
def _run_mask2former_inference(predictor, image_hwc: np.ndarray) -> PanopticOutput:
    """
    Run Mask2Former inference using the existing predictor and return PanopticOutput.
    This follows the same pattern as the working panoptic handling.
    
    Args:
        predictor: SatellitePredictor instance (from m2f_rs)
        image_hwc: Input image in HWC format
        
    Returns:
        PanopticOutput object containing model outputs
    """
    # Use the existing predictor to get model outputs (same as working panoptic handling)
    outputs = predictor(image_hwc)
    # Some predictors return a list[dict] even for a single image
    if isinstance(outputs, list) and len(outputs) > 0:
        outputs = outputs[0]
    
    # Mask2Former should always output panoptic format when configured properly
    if 'panoptic_seg' in outputs:
        return convert_mask2former_panoptic_output(outputs, image_id=None)
    else:
        raise ValueError(f"No panoptic_seg found in Mask2Former outputs. Available keys: {outputs.keys()}")
...
def run_mask2former_inference(
    ...
    output_dir: str,
    **kwargs
) -> Union[List[Detections], Tuple[List[Detections], List[PanopticOutput]]]:
    """
    Run inference using Mask2Former model following the m2f_rs setup pattern.
    ...
    # Import the existing setup utilities (same as working panoptic handling)
    import sys
    from pathlib import Path
    
    # Add the d2 models directory to path
    d2_path = Path(__file__).parent.parent / "src" / "models" / "d2"
    if str(d2_path) not in sys.path:
        sys.path.insert(0, str(d2_path))
    
    from mask2former_setup import setup_mask2former_config, get_satellite_predictor, preprocess_8channel_image
    
    # Setup config and predictor using existing utilities (same as working panoptic handling)
    cfg = setup_mask2former_config(config_file, model_weights, data_dir)
    predictor = get_satellite_predictor(cfg)
    
    # Create FTW dataset directly (same as other models)
    ...
    ds = FTW(
        ...
        transforms=None,  # We'll handle preprocessing manually
        load_boundaries=False,
        temporal_options=kwargs.get("temporal_options", "stacked"),
    )
    ...
    for i in tqdm(range(len(ds)), desc="Processing Mask2Former"):
        sample = ds[i]
        
        # Preprocess 8-channel image using existing utilities (same as working panoptic handling)
        image_hwc = preprocess_8channel_image(sample, cfg)
        
        # Run inference using existing predictor (same as working panoptic handling)
        logits = _run_mask2former_inference(predictor, image_hwc)
        
        # Store raw logits if requested
        if logits_list is not None:
            logits_list.append(logits)
        
        # Convert to Detections format using new intermediate format
        detections = logits.to_detections(
            min_area=kwargs.get("min_area", 0)
        )

        # Apply standardized postprocessing to align with baseline pipeline
        if kwargs.get("postprocess", False):
            post_cfg = kwargs.get("post_cfg", {})
            detections = detections.apply_standard_postprocess(
                min_area=post_cfg.get("min_area", 50),
                morph_kernel=post_cfg.get("morph_kernel", 3),
                morph_iterations=post_cfg.get("morph_iterations", 1),
                fill_holes=post_cfg.get("fill_holes", True),
                merge_iou=post_cfg.get("merge_iou", 0.0),
                simplify_tolerance=post_cfg.get("simplify_tolerance", 0.0),
            )
    ...
    with torch.inference_mode():
        ...
        for batch in pbar:
            ...
            for i in range(argmax_outputs.shape[0]):
                ...
                # Apply standardized postprocessing
                if kwargs.get("postprocess", False):
                    post_cfg = kwargs.get("post_cfg", {})
                    detections = detections.apply_standard_postprocess(
                        min_area=post_cfg.get("min_area", 50),
...
def run_sam_inference(
    ...
    **kwargs
    ...
    model_type = kwargs.get("model_type", "vit_h")  # Default to vit_h (not vit_b)
    ...
    print(f"[DEBUG] SAM model_type from kwargs: {kwargs.get('model_type')}, final: {model_type}", flush=True)
    
    # Match sam_controller.py behavior exactly (line 92):
    # - If change_chan_num=True: checkpoint is 3-channel, we'll replace patch_embed for 8-channel input
    # - If change_chan_num=False: checkpoint should match in_chans from config
    # 
    # However, we need to handle the case where in_chans=8 but checkpoint is 3-channel.
    # In sam_controller.py, if change_chan_num=False, it assumes checkpoint matches in_chans.
    # But if the checkpoint doesn't match, we'll get an error.
    #
    # The fix: always build with 3 channels if change_chan_num=True OR if in_chans=8 and checkpoint likely 3-channel
    ...
    for country in countries_sorted:
        ...
        for idx in tqdm(range(len(ds)), desc=f"SAM {country}"):
            ...
            # Optional standardized postprocess
            if kwargs.get("postprocess", False):
                post_cfg = kwargs.get("post_cfg", {})
                detections = detections.apply_standard_postprocess(
                    min_area=post_cfg.get("min_area", 50),
...
def run_torchgeo_inference(
    ...
    **kwargs
    ...
    """
    Run inference using generic torchgeo model.
    ...
    Returns:
        List of Detections objects
    """
    print("Running torchgeo inference...")
    ...
    # Load model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    trainer = BaseTask.load_from_checkpoint(model_weights, map_location="cpu")
    model = trainer.model.eval().to(device)
    ...
    with torch.inference_mode():
        for batch in tqdm(dl, desc="Processing torchgeo model"):
            images = batch["image"].to(device)
            ...
            # Convert to numpy - keep as logits if possible
            if hasattr(outputs, 'cpu'):
                raw_logits = outputs.cpu().numpy()
            else:
                # If already processed, convert back to logits
                raw_logits = np.log(outputs + 1e-8)  # Add small epsilon to avoid log(0)
            
            # Process each sample in the batch
            for i in range(raw_logits.shape[0]):
                # Create SemanticOutput from torchgeo outputs using the new logits system
                if raw_logits.ndim == 3:  # Single channel output (B, H, W)
                    sample_output = raw_logits[i]  # (H, W)
                else:  # Multi-channel output (B, C, H, W)
                    sample_output = raw_logits[i]  # (C, H, W)
                
                semantic_output = convert_baseline_output(
                    sample_output,
                    image_id=i
                )
                
                # Convert to Detections using new intermediate format
                detections = semantic_output.to_detections(
                    field_class_id=1,  # Field class is index 1
                    min_area=kwargs.get("min_area", 0)  # Changed from 50 to 0 for fair baseline comparison
                )
                detections_list.append(detections)
...
def run_delineate_anything_inference(
    ...
    **kwargs
    ...
    model_variant = kwargs.get("model_variant", None)
    ...
    for i in tqdm(range(len(ds)), desc="Processing DelineateAnything"):
        ...
        # Apply standardized postprocessing if requested
        if kwargs.get("postprocess", False):
            post_cfg = kwargs.get("post_cfg", {})
            detections = detections.apply_standard_postprocess(
                min_area=post_cfg.get("min_area", 50),
...
# Model registry
MODEL_RUNNERS = {
    "mask2former": run_mask2former_inference,
    "m2f": run_mask2former_inference,
    "ftw": run_ftw_inference,
    "ftw-cli": run_ftw_inference,
    "torchgeo": run_torchgeo_inference,
...
def main():
    ...
    parser.add_argument(
        "--config_file", 
        type=str,
        help="Path to model config file (for detectron2 models)"
    )
    ...
    parser.add_argument("--postprocess", action="store_true", help="Enable polygon postprocessing")
    parser.add_argument("--pp_min_area", type=int, default=0, help="Min area (pixels) to keep instances")
    parser.add_argument("--pp_morph_kernel", type=int, default=3, help="Morph kernel size for closing")
    parser.add_argument("--pp_morph_iterations", type=int, default=1, help="Morph closing iterations")
    parser.add_argument("--pp_fill_holes", action="store_true", default=True, help="Fill holes in instances")
    parser.add_argument("--pp_no_fill_holes", dest="pp_fill_holes", action="store_false")
    parser.add_argument("--pp_merge_iou", type=float, default=0.0, help="IoU threshold to merge instances")
    parser.add_argument("--pp_simplify_tolerance", type=float, default=0.0, help="Polygon simplify tolerance (px)")
    parser.add_argument(
        "--save_metadata", 
        action="store_true",
        default=True,
        help="Save metadata about the inference run"
    )
    parser.add_argument(
        "--save_logits", 
        action="store_true",
    ...
    runner_args = {
        ...
        "post_cfg": {
            "min_area": args.pp_min_area,
            "morph_kernel": args.pp_morph_kernel,
            "morph_iterations": args.pp_morph_iterations,
            "fill_holes": args.pp_fill_holes,
            "merge_iou": args.pp_merge_iou,
            "simplify_tolerance": args.pp_simplify_tolerance,
        },
    ...
    if args.model in ["mask2former", "m2f"] and not args.config_file:
        print(f"Error: --config_file is required for {args.model}")
        sys.exit(1)
    ...
    if args.model in ["delineate_anything", "da"] and not args.model_weights:
        ...
        if args.model_variant == "DelineateAnything-S":
