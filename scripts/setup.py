        ...
        try:
            ...
            if is_instance_seg:
                ...
                dataset_dicts = load_coco_panoptic_json(
                    paths["panoptic_json"], paths["image_root"], paths["panoptic_root"], meta
                )
    ...
    if getattr(args, "opts", None) and len(args.opts) > 0:
        try:
            ...
            if "Non-existent key" in error_msg:
                ...
                logger.error(
                    "Tip: Make sure all config keys exist. Use cfg.set_new_allowed(True) if you need to add new keys."
                )
