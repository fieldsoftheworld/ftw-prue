...
class FTWToCOCOConverter:
    ...
    def __init__(
        ...
        self.categories = [
            CategoryInfo(
                id=field_category_id, name="ag_field", supercategory="landcover", isthing=True, color=self.field_color
            ),
            CategoryInfo(
        ...
        for country in self.countries:
            ...
            for split in self.splits:
                ...
                if self.sample_fraction < 1.0:
                    split_chips = split_chips.sample(frac=self.sample_fraction, random_state=self.random_seed)
    ...
    def _create_instance(
        ...
        try:
            # Rasterize geometry
            mask = rasterize(
                [(geom, 1)], out_shape=(height, width), transform=transform, dtype=np.uint8, all_touched=True
            )
            ...
            return InstanceInfo(id=instance_id, mask=mask, bbox=bbox, area=area, color=color)
    ...
    def _process_single_image(
        self, country: str, aoi_id: str, split: str, image_id: int, generate_panoptic: bool = True
        ...
        try:
            ...
            if not is_negative_example:
                    ...
                    for idx, row in fields_gdf.iterrows():
                        ...
                        try:
                            instance = self._create_instance(
                                row.geometry, transform, height, width, annotation_id, generate_color=generate_panoptic
                            )
                        except Exception as e:
                ...
                # Process instance mask if using it
                    ...
                    for instance_id in instance_ids:
                        ...
                        instance = InstanceInfo(
                            id=annotation_id, mask=instance_binary, bbox=bbox, area=area, color=color
                        )
                ...
                # Process semantic mask (labeled connected components) if using it
                    ...
                    for label in range(1, num_labels + 1):
                        ...
                        # Create instance
                        instance = InstanceInfo(
                            id=annotation_id, mask=component_mask, bbox=bbox, area=area, color=color
                        )
            ...
            image_info = {
                ...
                "data_source": (
                    ...
                    if using_instance_mask
                    else f"mask_{mask_path_used}"
                    if using_mask
    ...
    def _generate_and_save_images(self, result: Dict[str, Any], split: str):
    ...
    def _process_country_split(
        self, country: str, split: str, aoi_ids: List[str], generate_panoptic: bool = True
            ...
            with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
                ...
                for aoi_id in chunk:
                    ...
                    future = executor.submit(
                        self._process_single_image, country, aoi_id, split, image_id, generate_panoptic
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Fields of the World dataset to COCO format")
    ...
    parser.add_argument(
        "--window", default="a", choices=["a", "b", "ab"], help="Which temporal window to use (a or b or ab)"
    )
