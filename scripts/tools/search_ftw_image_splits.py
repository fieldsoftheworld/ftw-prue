...
def get_ftw_image_info(
    data_root: str, country: str, split: str, index: int, window: str = "a", count: int = 1, show_all: bool = False
):
    """
    ...
    if index < 0 or index >= len(split_chips):
        raise ValueError(f"Index {index} out of range. Valid range: 0-{len(split_chips) - 1}")
...
Examples:
  ...
  %(prog)s --country austria --split test --index 10 --show_all
    ...
    parser.add_argument("--data_root", type=str, default="./data/ftw", help="Root directory of FTW dataset")
    parser.add_argument("--country", type=str, required=True, help="Country name (e.g., austria, france, germany)")
    parser.add_argument("--split", type=str, required=True, choices=["train", "val", "test"], help="Dataset split")
    parser.add_argument("--index", type=int, required=True, help="Zero-based index of the image in the split")
    parser.add_argument("--window", type=str, default="a", choices=["a", "b"], help="Temporal window (default: a)")
    parser.add_argument("--count", type=int, default=1, help="Number of images to retrieve (default: 1)")
    parser.add_argument("--show_all", action="store_true", help="Show all available columns from the GeoParquet")
