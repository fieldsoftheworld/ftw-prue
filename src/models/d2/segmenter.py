...
# Ensure vendored Detectron2 & Mask2Former are importable
_THIS_DIR = Path(__file__).resolve()
_PROJECT_ROOT = _THIS_DIR.parents[3]  # src/models/d2 -> src/models -> src -> repo
_DETECTRON2_ROOT = _PROJECT_ROOT / "detectron2"
_DETECTRON2_PKG = _DETECTRON2_ROOT / "detectron2"
_MASK2FORMER_PKG = _PROJECT_ROOT / "mask2former"

for _p in (_DETECTRON2_ROOT, _DETECTRON2_PKG, _MASK2FORMER_PKG):
    if _p.exists():
        sp = str(_p)
        if sp not in sys.path:
            sys.path.insert(0, sp)

from mask2former.config import add_maskformer2_config  # type: ignore  # noqa: E402
