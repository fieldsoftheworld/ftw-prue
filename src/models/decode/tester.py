        ...
        for country, loader in test_loaders.items():
            ...
            with torch.no_grad():
                for win_a, win_b, images, mask, boundary, distance in loader:
                    ...
                    preds = model(images)
                    pred_mask = torch.argmax(preds[0], dim=1).cpu().numpy()  # segmentation head
                    gt_mask = mask.squeeze(1).cpu().numpy()  # (N, H, W)

                    # -------------------------
                    # Presence-only adjustment
                    # -------------------------
...
# Build per-country test loaders
test_loaders = {}
