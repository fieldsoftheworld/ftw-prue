...
from src.eval.baseline_models import DeCurWrapper


def wrapper():
    return DeCurWrapper(
        weights_path=Path("data/baseline_models/decur"),
        modality="optical",  # or "SAR" for Sentinel-1
...
def process_country(country: str):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not os.path.exists(in_dir):
            continue
        out_dir = in_dir.replace("FTW-Dataset/ftw", "FTW-DECUR-Embeddings")
        out_dir = out_dir.replace("s2_images", "decur_optical")
