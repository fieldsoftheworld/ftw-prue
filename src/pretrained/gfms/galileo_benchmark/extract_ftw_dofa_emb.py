...
# Impute missing 4 with the mean
...
BASE_DIR = Path(__file__).parent
# Load DOFA model (base)
def wrapper():
    return DOFAWrapper(
        weights_path=Path("data/baseline_models/dofa"),
        size="large",  # or "large"
...
def process_country(country: str):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not os.path.exists(in_dir):
            continue
        out_dir = in_dir.replace("FTW-Dataset/ftw", "FTW-DOFA-Embeddings")
        out_dir = out_dir.replace("s2_images", "dofa_v1")
