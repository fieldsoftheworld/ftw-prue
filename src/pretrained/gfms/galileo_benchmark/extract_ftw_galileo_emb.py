...
BASE_DIR = Path(__file__).parent
SRC_DIR = BASE_DIR / "galileo" / "data" / "models" / "base"
def wrapper():
    return GalileoWrapper(
        pretrained_path=SRC_DIR, #Path("data/models/base"),
        patch_size=4,
        month=6,
        do_pool=False,
        load_state=False,
...
def process_country(country: str):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not os.path.exists(in_dir):
            continue
        out_dir = in_dir.replace("FTW-Dataset/ftw", "FTW-Galileo-Embeddings")
        out_dir = out_dir.replace("s2_images", "galileo_base")
