...
# Add galileo directory to path so it can be imported as a package
BASE_DIR = Path(__file__).parent
GALILEO_DIR = BASE_DIR / "galileo"
if str(GALILEO_DIR) not in sys.path:
    sys.path.insert(0, str(GALILEO_DIR))
...
from src.eval.baseline_models import PrithviWrapper
import sys

# Add galileo/src to path for imports
BASE_DIR = Path(__file__).parent
# Load Prithvi model
BASE_DIR = Path(__file__).parent
SRC_DIR = BASE_DIR / "galileo" / "data" / "baseline_models" #/ "prithvi"
def wrapper():
    return PrithviWrapper(
        weights_path=SRC_DIR, #Path("data/baseline_models/prithvi"),
        do_pool=False,  # Set to False for patch-level embeddings (segmentation)
        load_state=False,
    )#.to(device)
...
def process_country(country: str):
    ...
    for window in ["window_b", "window_a"]:
        ...
        if not os.path.exists(in_dir):
            continue
        out_dir = in_dir.replace("FTW-Dataset/ftw", "FTW-Prithvi-Embeddings")
        out_dir = out_dir.replace("s2_images", "prithvi_eo_v2")
