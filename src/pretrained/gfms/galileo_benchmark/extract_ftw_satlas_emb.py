...
from src.eval.baseline_models import SatlasWrapper
import sys
...
# Load Satlas model
def wrapper():
    return SatlasWrapper(
        weights_path=Path("data/baseline_models/satlas"),
        size="tiny",
        do_pool=False,
        load_state=False,
    )#.to(device)
...
def process_country(country: str):
    ...
    for window in [ "window_b", "window_a"]:
        ...
        if not os.path.exists(in_dir):
            continue
        out_dir = in_dir.replace("FTW-Dataset/ftw", "FTW-Satlas-Embeddings")
        out_dir = out_dir.replace("s2_images", "satlas_swin_tiny_ms")
