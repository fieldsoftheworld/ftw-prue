    ...
    def __init__(
        ...
        if size == "base":
            self.croma = PretrainedCROMA(
                str(weights_path / "CROMA_base.pt"), size, modality=modality, image_resolution=120, load_state=load_state
            )
            self.dim = 768
        elif size == "large":
            self.croma = PretrainedCROMA(
                str(weights_path / "CROMA_large.pt"), size, modality=modality, image_resolution=120, load_state=load_state
            )
            self.dim = 1024
        else:
...
class PretrainedCROMA(nn.Module):
    def __init__(
        self, pretrained_path="CROMA_base.pt", size="base", modality="both", image_resolution=120, load_state=True
    ):
        """
