...
class CustomTrainer(DefaultTrainer):
    ...
    def build_hooks(self):
        ...
        hooks.append(CheckpointCleanupHook(
            checkpoint_dir=self.cfg.OUTPUT_DIR,
            keep_last=5
        ))
        ...
        if self.cfg.TEST.EVAL_PERIOD > 0 and self.cfg.TEST.EVAL_PERIOD < self.cfg.SOLVER.MAX_ITER:
            ...
            hooks.append(PredictionVisualizationHook(
                self.cfg,
                self.model,
                test_loader,
    ...
    def build_test_loader(cls, cfg, dataset_name):
        if cfg.INPUT.DATASET_MAPPER_NAME == "coco_panoptic_custom":
            ...
            mapper = None
            return build_detection_test_loader(cfg, dataset_name, mapper=mapper, num_workers=cfg.DATALOADER.NUM_WORKERS, batch_size=cfg.TEST.BATCH_SIZE)
    ...
    def build_full_test_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "coco_panoptic_seg":
            ...
            evaluator_list.append(FTWEvaluator(
                dataset_name=dataset_name,
                distributed=True,
                output_dir=output_folder,
                country_names=country_names,
    ...
    def full_test(cls, cfg, model=None, eval_output_dir=None):
        ...
        # Get evaluator
        evaluator = cls.build_full_test_evaluator(
            cfg, cfg.DATASETS.TEST[0], output_folder=eval_output_dir
        )
    ...
    def test_with_evaluator(cls, model, data_loader, evaluator):
        ...
        with torch.no_grad():
            ...
            for idx, inputs in enumerate(data_loader):
                ...
                if idx % 50 == 0:
                    if target_countries:
                        logger.info(f"Processed {num_processed} samples, skipped {num_skipped} samples from non-target countries")
                    else:
