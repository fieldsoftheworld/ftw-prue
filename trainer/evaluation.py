...
def get_object_level_metrics_panoptic(
    gt_panoptic, gt_segments_info, pred_panoptic, pred_segments_info, iou_threshold=0.5
):
        ...
        if segment.get("isthing", True):  # and segment.get("category_id") == 0
            ...
            if mask.any():
                ...
                geoms = [
                    ...
                    for geom, _ in rasterio.features.shapes(mask.astype(np.uint8), mask=(mask))
                ]
                for geom in geoms:
...
class FTWEvaluator(DatasetEvaluator):
        ...
        for input_record, output in zip(inputs, outputs):
            ...
            try:
                if panoptic_for_object is not None and gt_panoptic is not None:
                    ...
                    tps, fps, fns = get_object_level_metrics(gt_binary, pred_binary, iou_threshold=self._iou_threshold)
                    ...
                    if segment.get("isthing", True):  # check if it's a thing class
                        ...
                        if mask.sum() > 10:
                            ...
                            coco_instances.append(
                                {
                                    "image_id": input_record[
                                        "image_id"
                                    ],  # file_to_id.get(input_record["file_name"], input_record["image_id"]),  # Use the image_id from input record to match ground truth
    ...
    def evaluate(self):
            ...
            if "pixel" in self._metrics:
                ...
                if self._country_names:
                    country_confusion_matrices = {
                        country: np.zeros_like(self._confusion_matrix) for country in self._country_names
                    }
        ...
        if "pixel" in self._metrics:
            pixel_results = self._compute_metrics(confusion_matrix, total_tps, total_fps, total_fns, prefix="")
        ...
        if "coco" in self._metrics and len(predictions) > 0:
            ...
            if len(coco_gt.getAnnIds()) == 0:
                ...
                self._logger.error(
                    f"Predicted image_ids: {list(evaluated_image_ids)[:10] if evaluated_image_ids else 'None'}"
                )
                self._logger.error(
                    f"Predicted file_names: {list(evaluated_file_names)[:10] if evaluated_file_names else 'None'}"
                )
            ...
            for pred in coco_results:
                ...
                if pred_file_name and pred_file_name in file_name_to_gt_image_id:
                    ...
                    if pred_image_id != gt_image_id:
                        self._logger.debug(
                            f"Remapping prediction image_id {pred_image_id} -> {gt_image_id} for file {pred_file_name}"
                        )
    ...
    def _summarize_coco_metrics(self, eval_results):
        ...
        stats[1] = _summarize(1, iouThr=0.5)  # AP @[ IoU=0.50      | area=   all | maxDets=100 ]
        stats[2] = _summarize(1, iouThr=0.75)  # AP @[ IoU=0.75      | area=   all | maxDets=100 ]
    ...
    def _compute_coco_metrics(self, coco_gt, coco_dt, iou_type):
        ...
        if getattr(self, "_enable_coco_debug", False):
            debug_dir = (
                os.path.join(self._output_dir, "debug_visualizations") if self._output_dir else "debug_visualizations"
            )
            import random
    ...
    def _prepare_detailed_results(self, eval_results, stats):
        ...
        params_dict = {
            ...
            "maxDets": eval_results["params"].maxDets.tolist()
            if hasattr(eval_results["params"].maxDets, "tolist")
            else list(eval_results["params"].maxDets),
            "areaRng": eval_results["params"].areaRng,
            "areaRngLbl": eval_results["params"].areaRngLbl,
            "useCats": eval_results["params"].useCats,
            "catIds": [int(x) for x in eval_results["params"].catIds]
            if eval_results["params"].catIds is not None
            else None,
            "imgIds": [int(x) for x in eval_results["params"].imgIds]
            if eval_results["params"].imgIds is not None
            else None,
        }
    ...
    def _create_coco_gt_with_rle(self, selected_image_ids: set = None, selected_file_names: set = None):
        ...
        categories = [
            {"id": 1, "name": "ag_field", "supercategory": "landcover", "isthing": 1},
            {"id": 2, "name": "background", "supercategory": "background", "isthing": 0},
        ]

        # Create COCO dataset structure
        coco_dataset = {
            "info": {"description": "Fields of The World Dataset", "version": "1.0", "year": 2024},
            "licenses": [],
            "images": images,
