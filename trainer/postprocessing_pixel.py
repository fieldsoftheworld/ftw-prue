...
def filter_segments_by_isthing(
    segments_info: List[Dict],
    keep_things: bool = True
) -> List[Dict]:
    """
...
def filter_panoptic_segments_pixel_space(
    ...
    filtered_segments_info = filter_segments_by_confidence(
        segments_info, confidence_threshold
    )
    ...
    filtered_segments_info = filter_segments_by_category(
        filtered_segments_info, category_id=0, exclude_background=True
    )
    
    # Step 3: Filter by isthing (keep only instances)
    filtered_segments_info = filter_segments_by_isthing(
        filtered_segments_info, keep_things=True
    )
    ...
    for segment in filtered_segments_info:
        ...
        filtered_mask, is_valid = filter_mask_by_area_pixels(
            mask, min_area_pixels, max_area_pixels
        )
        if not is_valid:
            continue
        ...
        if remove_small_objects_size is not None:
            ...
            filtered_mask, is_valid = filter_mask_by_area_pixels(
                filtered_mask, min_area_pixels, max_area_pixels
            )
            if not is_valid:
                continue
