...
os.environ['PYKEOPS_BACKEND'] = "GPU"
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=32, norm_layer=None): # revised in_chans to 4 as default
        super().__init__()
        self.patch_size = 4
...
class AFF(nn.Module):
    ...
    """
    # changed in_chans to 4
    def __init__(self, in_chans=4, embed_dim=[32, 128, 256, 512],
    ...
    # # downsampling maps every ~20 iters, save viz in subfolder
    # # put a pdb here to take a look at pos to get token positions, add to outs to write to TB
    # def forward(self, x):
    #     '''
    #     x: b x c x h x w - mean-normalized images, e.g. [128, 4, 192, 192] for batch size 128 and 4-channel BGRN 192x192 images

    #     b=128, c=4, h=192, w=192
    #     '''
    #     pos, x, h, w = self.patch_embed(x)

    #     '''
    #     pos: b x n x c, e.g. [128, 2304, 2]
    #     x: b x n x d e.g. [128, 2304, 96]
    #     h, w: e.g. 48

    #     b=batch=128, n=tokens=2304(=48*48), c=feature channel=2, d=position channel=96(=192/2)
    #     '''
    #     x = self.pos_drop(x) # dropout; shape remains b x n x d
    #     spatial_shape = (h, w)
    ...
    #     for i_layer in range(self.num_layers):
    #         layer = self.layers[i_layer] #BasicLayer
    #         pos_out, x_out, pos, x = layer(pos, x, h=h, w=w, on_grid=i_layer == 0, stride=2**(i_layer+1))

    #         '''
    #         Multiply pos by 4 to get img coordinate
    #         pos_out: b x (h*w) x c, e.g. [128, 2304, 2] where h,w=48. pos_out[:,:,0] is x-positions, pos_out[:,:,1] is y-positions.
    #         x_out: b x (h*w) x d, e.g. [128, 2304, 96]
    #         pos: b x n x c, e.g. [128, 576, 2]
    #         x: b x (h*w) x ??, e.g. [128, 576, 192], where 24*24=576 after downsampling, 192 was the image h and w

    #         b=128, h,w=48 (=192/4), c=2, d=96, n=576(=2304/4)
    #         '''
    #         with tempfile.TemporaryDirectory() as dirname:                
    #             # blank tensor
    #             token_map = torch.zeros(h,w).flatten() # 48*48=[2304], goal is to have 1's where the tokens are (given by indices pos_out[0,:,0] and pos_out[0,:,1] for first image in batch)

    #             # position
    #             idx = pos_out[0,:,0] + (w)*pos_out[0,:,1] #1D flattened indices of tokens, shape [2304]. But if it's an array of indices what do the indices of the array itself mean?
    #             token_map[idx.to(torch.int)] = 1 # this just fills the entire map though?

    #             token_map = token_map.reshape((h,w))

    #             token_map = F.interpolate(token_map.unsqueeze(0).unsqueeze(0), size=(h*4,w*4), mode='nearest')
    #             rgb = x[0,:3,:,:][::-1,:,:].astype(np.float32)
    #             for i in range(3):
    #                 band = rgb[i,:,:]
    #                 if band.max() > band.min():
    #                     rgb[i,:,:]=255*(band-band.min())/(band.max() - band.min()) # return to uint8
    #             torchvision.save_image(rgb, 'test_forward.png')

    #             red = torch.zeros_like(rgb)
    #             red[0,:,:]=1 # set to all-red
    #             out = red*token_map + rgb*(1-token_map)
    #             torchvision.save_image(out, 'test_tokens.png')
    ...
    # def forward(self, x):
    #     """
    #     Forward pass with minimal token visualization
        
    #     Args:
    #         x: Tensor of shape (B,C,H,W) - input images
                
    #     Returns:
    #         dict[str->Tensor]: Feature maps
    #     """
    #     # Store original input for visualization
    #     orig_input = x.clone()
    #     orig_height, orig_width = orig_input.shape[2], orig_input.shape[3]
    ...
    #             # Store features and positions in output dict
    #             outs[f"res{i_layer + 2}"] = x_out
    #             outs[f"res{i_layer + 2}_pos"] = pos_out
    #             outs[f"res{i_layer + 2}_spatial_shape"] = spatial_shape
        
    #     # Simple token visualization every 10K iterations
    #     import os
    #     import torch.distributed as dist
        
    #     # Create or update a global iteration counter
    #     if not hasattr(self, '_iter_counter'):
    #         self._iter_counter = 0
    #     else:
    #         self._iter_counter += 1
        
    #     # Only visualize every 10K iterations and on main process
    #     if dist.is_initialized():
    #         is_main = dist.get_rank() == 0
    #     else:
    #         is_main = True
        
    #     if is_main and self._iter_counter % 100 == 0:
    #         try:
    #             import matplotlib.pyplot as plt
    #             import numpy as np
                
    #             # Create output directory
    #             viz_dir = os.path.join(os.getcwd(), 'token_visualizations')
    ...
    #             # Extract the image
    #             image = orig_input[img_idx].detach().cpu().numpy()
                
    #             # Create RGB image for visualization (assuming 4-channel satellite image)
    #             if image.shape[0] >= 3:
    #                 # Get RGB from first 3 channels (BGR format, convert to RGB)
    ...
    #                 rgb_image = np.stack([image[0], image[0], image[0]])
    #                 if rgb_image[0].max() > rgb_image[0].min():
    #                     rgb_image = (rgb_image - rgb_image.min()) / (rgb_image.max() - rgb_image.min())
                
    #             # Transpose from CxHxW to HxWxC for plotting
    ...
    #             # For each stage, create a token map visualization
    #             for stage_idx in range(4):
    #                 # Get token positions
    #                 if stage_idx == 0:
    #                     # First stage is from initial embedding
    #                     pos_tensor = pos.clone()[img_idx].detach().cpu().numpy()
    #                     scale_factor = 4  # First stage is downsampled by 4
    #                     num_tokens = pos_tensor.shape[0]
    #                     stage_name = f"Stage {stage_idx+1} - {num_tokens} tokens"
    #                 else:
    #                     # Later stages from layer outputs
    #                     key = f"res{stage_idx + 2}_pos"
    #                     if key in outs:
    #                         pos_tensor = outs[key][img_idx].detach().cpu().numpy()
    #                         scale_factor = 2**(stage_idx+1)  # 8, 16, 32
    #                         num_tokens = pos_tensor.shape[0]
    #                         stage_name = f"Stage {stage_idx+1} - {num_tokens} tokens"
    #                     else:
    #                         # Skip if not available
    #                         axes[stage_idx+1].set_title(f"Stage {stage_idx+1} - N/A")
    #                         axes[stage_idx+1].axis('off')
    #                         continue
                    
    #                 # Create a copy of original image for this stage
    #                 img_with_tokens = rgb_image.copy()
                    
    #                 # Scale positions to image coordinates
    #                 pos_scaled = pos_tensor * scale_factor
                    
    #                 # Draw tokens on the image
    #                 for pos in pos_scaled:
    #                     x, y = int(pos[0]), int(pos[1])
    #                     if 0 <= x < orig_width and 0 <= y < orig_height:
    #                         # Draw a 3x3 square for visibility
    #                         y1, y2 = max(0, y-1), min(orig_height-1, y+1)
    #                         x1, x2 = max(0, x-1), min(orig_width-1, x+1)
                            
    #                         # Set pixels to red
    #                         img_with_tokens[y1:y2+1, x1:x2+1, 0] = 1.0  # Red
    #                         img_with_tokens[y1:y2+1, x1:x2+1, 1] = 0.0  # Green
    #                         img_with_tokens[y1:y2+1, x1:x2+1, 2] = 0.0  # Blue
                    
    #                 # Plot
    #                 axes[stage_idx+1].imshow(img_with_tokens)
    #                 axes[stage_idx+1].set_title(stage_name)
    #                 axes[stage_idx+1].axis('off')
                
    #             # Save figure
    #             plt.tight_layout()
    #             fig_path = os.path.join(viz_dir, f"tokens_iter{self._iter_counter}.png")
    #             plt.savefig(fig_path, dpi=150, bbox_inches='tight')
    #             plt.close(fig)
                
    #             print(f"[AFF] Token visualization saved to {fig_path}")
                
    #         except Exception as e:
    #             print(f"[AFF] Token visualization error: {str(e)}")
    #             import traceback
    #             traceback.print_exc()
    ...
    def forward(self, x):
        """
        Forward pass with visualization of actual visible tokens at each stage.
        
        Args:
            x: Tensor of shape (B,C,H,W) - input images
                
        Returns:
            dict[str->Tensor]: Feature maps at different resolutions
        """
        # Store original input for visualization
        orig_input = x.clone()
        orig_height, orig_width = orig_input.shape[2], orig_input.shape[3]
        ...
        # Store the initial tokens after patch embedding (Stage 1)
        stage_tokens = [pos]  # Stage 1 tokens
        ...
        for i_layer in range(self.num_layers):
            ...
            # Store the downsampled tokens after this layer
            # pos represents the tokens passed to the next stage
            stage_tokens.append(pos)
            
            if i_layer in self.out_indices:
                norm_layer = getattr(self, f"norm{i_layer}")
                x_out = norm_layer(x_out)
                
                # Store in output dict with appropriate stage names
                stage_name = f"res{i_layer + 2}"
                outs[stage_name] = x_out
                outs[f"{stage_name}_pos"] = pos_out
                outs[f"{stage_name}_spatial_shape"] = spatial_shape
        ...
        # Only visualize every 1000 iterations and on main process
        if dist.is_initialized():
            is_main = dist.get_rank() == 0
        else:
            is_main = True
        ...
        if is_main and self._iter_counter % 100 == 0:
            try:
                ...
                # Extract the original input image
                image = orig_input[img_idx].detach().cpu().numpy()
                
                if image.shape[0] >= 3:
                    # BGR to RGB conversion (if first 3 channels are BGR)
                    rgb_image = np.zeros((3, orig_height, orig_width), dtype=np.float32)
                    for c in range(3):
                        channel_data = image[c]
                        if channel_data.max() > channel_data.min():
                            rgb_image[2-c] = (channel_data - channel_data.min()) / (channel_data.max() - channel_data.min())
                        else:
                            rgb_image[2-c] = channel_data
                else:
                    # Handle grayscale case
                    rgb_image = np.zeros((3, orig_height, orig_width), dtype=np.float32)
                    ...
                    rgb_image[0] = rgb_image[1] = rgb_image[2] = normalized
                ...
                # For each stage, visualize the tokens
                for stage_idx in range(4):
                    # Get token positions for this stage
                    pos_tensor = stage_tokens[stage_idx][img_idx].detach().cpu().numpy()
                    
                    # Create a copy of original image for this stage
                    img_with_tokens = rgb_image.copy()
                    
                    # Scale positions to image coordinates
                    # scale_factor = scale_factors[stage_idx]
                    scale_factor = 4
                    pos_scaled = pos_tensor * scale_factor
                    
                    # Keep track of visible tokens only
                    visible_tokens = 0
                    ...
                    for pos in pos_scaled:
                        x, y = int(pos[0]), int(pos[1])
                        if 0 <= x < orig_width and 0 <= y < orig_height:
                            # Draw a small red dot for each token
                            y_min, y_max = max(0, y-1), min(orig_height-1, y+1)
                            x_min, x_max = max(0, x-1), min(orig_width-1, x+1)
                            img_with_tokens[y_min:y_max+1, x_min:x_max+1, 0] = 1.0  # Red
                            img_with_tokens[y_min:y_max+1, x_min:x_max+1, 1] = 0.0  # Green
                            img_with_tokens[y_min:y_max+1, x_min:x_max+1, 2] = 0.0  # Blue
                            
                            # Increment visible token count
                            visible_tokens += 1
                    
                    # Only show actual visible tokens in the title
                    axes[stage_idx+1].imshow(img_with_tokens)
                    axes[stage_idx+1].set_title(f"Stage {stage_idx+1} - {visible_tokens} tokens")
                    axes[stage_idx+1].axis('off')
                    
                    # Optional: Print comparison of total tokens vs visible tokens
                    total_tokens = pos_tensor.shape[0]
                    print(f"Stage {stage_idx+1}: {visible_tokens} visible tokens out of {total_tokens} total tokens")
