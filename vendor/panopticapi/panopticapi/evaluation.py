...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories):
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        # TODO replace this quick hack: chop off the .npz extension from the file name and add .png
        gt_file_name = gt_ann['file_name'].replace('.npz', '.png')
        # pan_gt = np.array(Image.open(os.path.join(gt_folder, gt_ann['file_name'])), dtype=np.uint32)
        pan_gt = np.array(Image.open(os.path.join(gt_folder, gt_file_name)), dtype=np.uint32)
        pan_gt = rgb2id(pan_gt)
        pan_pred = np.array(Image.open(os.path.join(pred_folder, gt_file_name)), dtype=np.uint32)
        # pan_pred = np.array(Image.open(os.path.join(pred_folder, gt_ann['file_name'])), dtype=np.uint32)
